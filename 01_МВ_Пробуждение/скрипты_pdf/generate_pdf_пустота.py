#!/usr/bin/env python3
"""
Мирный Воин. Пробуждение — ПУСТОТА
Читает текст из КНИГА_ПУСТОТА_текст.md и верстает PDF
...
def cover_cb(cv, doc):
    ...
    cv.drawCentredString(W/2, H/2 - 20*mm, "Когда всё достигнуто. И непонятно — зачем")
    # тег ЯМА
    ...
    cv.drawCentredString(W/2, ry + 6, "ПУСТОТА  ·  7%")
    # нижний орнамент
    cv.setStrokeColor(GOLD); cv.setLineWidth(0.4)
...
def page_cb(cv, doc):
    ...
    cv.drawRightString(W - MR, H - 14*mm, "ПРОБУЖДЕНИЕ · ПУСТОТА")
...
def build_pdf(text_file, output_path):
    ...
    # ── ОБЛОЖКА ──
    cover_path = "/tmp/mvs_cover_pustota.pdf"
    c = rl_canvas.Canvas(cover_path, pagesize=A4)
    c.setTitle("Мирный Воин. Пробуждение — ПУСТОТА")
    ...
    # ── КОНТЕНТ ──
    content_path = "/tmp/mvs_content_pustota.pdf"
    ...
    doc = BaseDocTemplate(
        ...
        title="Мирный Воин. Пробуждение — ПУСТОТА",
...
if __name__ == "__main__":
    TEXT_FILE = "/home/user/workspace/КНИГА_ПУСТОТА_текст.md"
    OUTPUT = "/home/user/workspace/МВ_Пробуждение_ПУСТОТА.pdf"
