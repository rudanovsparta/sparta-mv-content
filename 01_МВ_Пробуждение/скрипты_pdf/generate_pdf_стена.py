#!/usr/bin/env python3
"""
Мирный Воин. Пробуждение — СТЕНА v2
Читает текст из КНИГА_СТЕНА_текст_v2.md и верстает PDF
...
def cover_cb(cv, doc):
    ...
    cv.drawCentredString(W/2, H/2 - 20*mm, "Для тех, кто знает что делать — и не делает")
    # тег СТЕНА
    ...
    cv.drawCentredString(W/2, ry + 6, "СТЕНА  ·  48%")
    # нижний орнамент
    cv.setStrokeColor(GOLD); cv.setLineWidth(0.4)
...
def page_cb(cv, doc):
    ...
    cv.drawRightString(W - MR, H - 14*mm, "ПРОБУЖДЕНИЕ · СТЕНА")
...
def build_pdf(text_file, output_path):
    ...
    # ── ОБЛОЖКА ──
    cover_path = "/tmp/mvs_cover_v2.pdf"
    c = rl_canvas.Canvas(cover_path, pagesize=A4)
    c.setTitle("Мирный Воин. Пробуждение — СТЕНА v2")
    ...
    # ── КОНТЕНТ ──
    content_path = "/tmp/mvs_content_v2.pdf"
    ...
    doc = BaseDocTemplate(
        ...
        title="Мирный Воин. Пробуждение — СТЕНА v2",
...
if __name__ == "__main__":
    TEXT_FILE = "/home/user/workspace/КНИГА_СТЕНА_текст_v2.md"
    OUTPUT = "/home/user/workspace/МВ_Пробуждение_СТЕНА_v2.pdf"
