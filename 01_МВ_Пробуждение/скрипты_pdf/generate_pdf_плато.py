#!/usr/bin/env python3
"""
Мирный Воин. Пробуждение — ПЛАТО
Читает текст из КНИГА_ПЛАТО_текст.md и верстает PDF
...
def cover_cb(cv, doc):
    ...
    cv.drawCentredString(W/2, H/2 - 20*mm, "Для тех, кто достиг результата — и упёрся в потолок")
    # тег ПЛАТО
...
def build_pdf(text_file, output_path):
    ...
    # ── ОБЛОЖКА ──
    cover_path = "/tmp/mvs_cover_plato.pdf"
    c = rl_canvas.Canvas(cover_path, pagesize=A4)
    c.setTitle("Мирный Воин. Пробуждение — ПЛАТО")
    ...
    # ── КОНТЕНТ ──
    content_path = "/tmp/mvs_content_plato.pdf"
    ...
    doc = BaseDocTemplate(
        ...
        title="Мирный Воин. Пробуждение — ПЛАТО",
...
if __name__ == "__main__":
    TEXT_FILE = "/home/user/workspace/КНИГА_ПЛАТО_текст.md"
    OUTPUT = "/home/user/workspace/МВ_Пробуждение_ПЛАТО.pdf"
