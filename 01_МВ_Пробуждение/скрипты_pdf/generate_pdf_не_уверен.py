#!/usr/bin/env python3
"""
Мирный Воин. Пробуждение — НЕ УВЕРЕН
Читает текст из КНИГА_НЕ_УВЕРЕН_текст.md и верстает PDF
...
def cover_cb(cv, doc):
    ...
    cv.drawCentredString(W/2, H/2 - 20*mm, "Когда запросов несколько — и ни один не главный")
    # тег ЯМА
    ...
    cv.drawCentredString(W/2, ry + 6, "НЕ УВЕРЕН")
    # нижний орнамент
    cv.setStrokeColor(GOLD); cv.setLineWidth(0.4)
...
def page_cb(cv, doc):
    ...
    cv.drawRightString(W - MR, H - 14*mm, "ПРОБУЖДЕНИЕ · НЕ УВЕРЕН")
...
def build_pdf(text_file, output_path):
    ...
    # ── ОБЛОЖКА ──
    cover_path = "/tmp/mvs_cover_ne_uveren.pdf"
    c = rl_canvas.Canvas(cover_path, pagesize=A4)
    c.setTitle("Мирный Воин. Пробуждение — НЕ УВЕРЕН")
    ...
    # ── КОНТЕНТ ──
    content_path = "/tmp/mvs_content_ne_uveren.pdf"
    ...
    doc = BaseDocTemplate(
        ...
        title="Мирный Воин. Пробуждение — НЕ УВЕРЕН",
...
if __name__ == "__main__":
    TEXT_FILE = "/home/user/workspace/КНИГА_НЕ_УВЕРЕН_текст.md"
    OUTPUT = "/home/user/workspace/МВ_Пробуждение_НЕ_УВЕРЕН.pdf"
