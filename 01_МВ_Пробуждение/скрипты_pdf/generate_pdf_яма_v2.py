...
Читает текст из КНИГА_ЯМА_v2.md и верстает PDF
...
def cover_cb(cv, doc):
    ...
    cv.drawCentredString(W/2, ry + 6, "ЯМА  ·  8%")
    # нижний орнамент
    cv.setStrokeColor(GOLD); cv.setLineWidth(0.4)
...
def page_cb(cv, doc):
    ...
    cv.drawRightString(W - MR, H - 14*mm, "ПРОБУЖДЕНИЕ · ЯМА")
...
def build_pdf(text_file, output_path):
    ...
    # ── ОБЛОЖКА ──
    cover_path = "/tmp/mvs_cover_яма_v2.pdf"
    ...
    # ── КОНТЕНТ ──
    content_path = "/tmp/mvs_content_яма_v2.pdf"
...
if __name__ == "__main__":
    TEXT_FILE = "/home/user/workspace/КНИГА_ЯМА_v2.md"
    OUTPUT = "/home/user/workspace/МВ_Пробуждение_ЯМА_v2.pdf"
